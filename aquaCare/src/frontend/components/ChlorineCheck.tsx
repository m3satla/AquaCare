...
const ChlorineCheck = () => {
  ...
  const loadSensorHistory = async () => {
    try {
      ...
      // נסה לטעון היסטוריה ספציפית לחיישן כלור
      const response = await axios.get("http://localhost:5001/api/sensors/history/chlorine");
      ...
      if (response.data.success) {
        ...
        const convertedHistory = allSensors.map((sensor: any) => ({
          ...
          value: sensor.chlorine?.toString() || "0",
          label: "רמת כלור",
          sensorType: "chlorine",
  ...
  const checkSensor = async () => {
    try {
      ...
      const value = (Math.random() * (3.0 - 2.0) + 2.0).toFixed(1);
      const now = new Date().toLocaleString("he-IL");
      const label = "רמת כלור";
  ...
  return (
    <Box p={4}>
      <Typography variant="h4" gutterBottom>
        {t('sensors.chlorineLevel', 'רמת כלור')}
      </Typography>
              ...
              {history.length === 0 ? (
                ...
                history.map((entry) => (
                  <TableRow key={entry._id}>
                    ...
                    <TableCell>{entry.value} ppm</TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
